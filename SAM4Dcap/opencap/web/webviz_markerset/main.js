...
const elements = {
  ...
  showMarkerLabels: document.getElementById("showMarkerLabels"),
  markerScene: document.getElementById("markerScene"),
  loadMarkers: document.getElementById("loadMarkers"),
...
async function loadTrial(session, trial) {
  const jsonUrl = `/output/Data/${encodeURIComponent(session)}/VisualizerJsons/${encodeURIComponent(trial)}/${encodeURIComponent(trial)}.json`;
...
async function loadMarkerScene(scenePath) {
  const url = scenePath?.trim() || "";
  if (!url) throw new Error("Marker JSON path is empty");
  const res = await fetch(url);
  if (!res.ok) {
    throw new Error(`Failed to fetch marker JSON: ${res.status} ${res.statusText} (${url})`);
  }
  return res.json();
}
...
function markerColor(name) {
  ...
  if (n.includes("thigh") || n.includes("knee") || n.includes("ankle") || n.includes("toe") || n.includes("calc") || n.includes("meta"))
    return 0x7cffa7;
  if (n.includes("shoulder") || n.includes("elbow") || n.includes("wrist") || n.includes("_sh")) return 0xffc27a;
  return 0xd6d6d6;
}

function setMarkersMessage(text) {
  elements.markerList.replaceChildren();
  const div = document.createElement("div");
  div.className = "empty";
  div.textContent = text;
  elements.markerList.appendChild(div);
}
...
function buildVideoGrid(session, trial) {
  ...
  const promises = cams.map(async (cam) => {
    const url = `/output/Data/${encodeURIComponent(session)}/VisualizerVideos/${encodeURIComponent(trial)}/${encodeURIComponent(trial)}_syncd_${cam}.mp4`;
...
function buildScene(container) {
  ...
  return { scene, camera, renderer, labelRenderer, controls, modelGroup };
}
...
function buildMarkerObjects(scene, markers, pelvisTranslation, radius, showLabels) {
  const group = new THREE.Group();
  ...
  const pelvis = Array.isArray(pelvisTranslation) && pelvisTranslation.length >= 3 ? pelvisTranslation : [0, 0, 0];

  for (const marker of markers) {
    const name = marker?.name;
    const position = marker?.position;
    if (!name || !Array.isArray(position) || position.length < 3) continue;
    ...
    // Store marker positions relative to the pelvis origin (generic model),
    // so we can place the entire group onto the current VisualizerJsons pelvis.
    mesh.position.set(position[0] - pelvis[0], position[1] - pelvis[1], position[2] - pelvis[2]);
...
function renderMarkerList(markerMap) {
  ...
  if (!names.length) {
    setMarkersMessage("No markers loaded (list is empty)");
    return;
  }
...
async function start() {
  ...
  function alignMarkersToSkeleton(frameIndex = 0) {
    if (!markersOverlay.loaded || !markersOverlay.group) return;
    if (!playback.loaded || !playback.state) return;
    const pelvisT = playback.state.bodyTranslations.get("pelvis")?.[frameIndex];
    const pelvisR = playback.state.bodyRotations.get("pelvis")?.[frameIndex];
    if (!pelvisT) return;
    markersOverlay.group.position.set(pelvisT[0], pelvisT[1], pelvisT[2]);
    if (pelvisR) {
      markersOverlay.group.rotation.set(pelvisR[0], pelvisR[1], pelvisR[2], "XYZ");
    } else {
      markersOverlay.group.rotation.set(0, 0, 0);
    }
  }

  function clearMarkers() {
    if (markersOverlay.group) viz.scene.remove(markersOverlay.group);
    markersOverlay = { loaded: false, group: null, markerMap: new Map(), pelvisGeneric: [0, 0, 0] };
    elements.markerList.replaceChildren();
  }

  function updateMarkerLabelsVisible() {
    if (!markersOverlay.loaded) return;
    const show = Boolean(elements.showMarkerLabels?.checked);
    for (const entry of markersOverlay.markerMap.values()) entry.label.visible = show;
  }

  function updateMarkerSize() {
    if (!markersOverlay.loaded) return;
    const r = Number(elements.markerSize?.value || "0.02");
    for (const entry of markersOverlay.markerMap.values()) {
  ...
  async function loadAndShowMarkers() {
    const scenePath = elements.markerScene?.value?.trim() || "";
    clearMarkers();
    setMarkersMessage("Loading marker JSON...");

    const data = await loadMarkerScene(scenePath);
    const markers = Array.isArray(data?.markers) ? data.markers : [];
    const pelvisGeneric = data?.bodies?.pelvis?.translation;
    if (Array.isArray(pelvisGeneric) && pelvisGeneric.length >= 3) {
      markersOverlay.pelvisGeneric = pelvisGeneric;
    }
    const radius = Number(elements.markerSize?.value || "0.02");
    const showLabels = Boolean(elements.showMarkerLabels?.checked);

    const built = buildMarkerObjects(viz.scene, markers, markersOverlay.pelvisGeneric, radius, showLabels);
    markersOverlay.loaded = true;
    markersOverlay.group = built.group;
    markersOverlay.markerMap = built.markerMap;
    markersOverlay.group.visible = Boolean(elements.showMarkers?.checked);

    renderMarkerList(markersOverlay.markerMap);
    applyMarkerFilter();

    // Align once onto the current skeleton (static overlay).
    alignMarkersToSkeleton(playback.frameIndex ?? 0);
  }
  ...
  elements.loadMarkers?.addEventListener("click", () => loadAndShowMarkers().catch((e) => setMarkersMessage(String(e))));
  elements.showMarkers?.addEventListener("change", () => {
    if (!markersOverlay.loaded || !markersOverlay.group) return;
    markersOverlay.group.visible = Boolean(elements.showMarkers?.checked);
  });
  elements.showMarkerLabels?.addEventListener("change", updateMarkerLabelsVisible);
  elements.markerSize?.addEventListener("input", updateMarkerSize);
  ...
  await loadAndShowMarkers().catch((e) => setMarkersMessage(String(e)));
