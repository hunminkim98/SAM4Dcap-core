#!/usr/bin/env python3
"""
Generate a 43-marker aligned TRC (based on smpl_results) and output to this directory:
- output/aligned_subject2_motion_43.trc
- webviz_compare/data.json (for visualization, reuse align/webviz_compare/index.html)
"""
...
def main() -> None:
    ...
    markers_yaml = root / "markers_lai_43.yaml"
    ...
    markers43_world_frames: List[np.ndarray] = []
    ...
    for i in range(max_frames):
        ...
        markers43_world_frames.append(pts_world)
    ...
    n_frames = min(
        ...
        len(markers43_world_frames) - sam_start,
    ...
    markers43_world_frames = markers43_world_frames[sam_start : sam_start + n_frames]
    ...
    for i in range(n_frames):
        ...
        marker_frame = markers43_world_frames[i]
    ...
    for seq in (mhr_points, smpl_points, markers_points):
        for frame in seq:
            frame[:, 1] += offset_y
    ...
    out = {
        ...
        "temporal_lag_frames": int(lag_all),
    ...
    trc_out_path = trc_out_dir / "aligned_subject2_motion_43.trc"
    write_trc(trc_out_path, marker_names, markers_points, frame_rate)

    print(
        f"[OK] wrote {out_path} with {n_frames} frames "
        f"(lag={lag_all}, offset_y={offset_y:.4f}, scale={scale:.4f})"
    )
    print(f"[OK] wrote aligned TRC (43 markers): {trc_out_path}")
